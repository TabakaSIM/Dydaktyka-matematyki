...
\begin{document}
	...
	\begin{enumerate}[1)]
		...
		\item $\exists_{e\in G} \; \forall_{a\in G} \; c*a=a*e=a$ (istnieje element neutralny)
	...
	\Odp{
		...
		$\exists_{1\in R} \forall a\cdot1=1\cdot a=a$
	}
	
	\item Podać definicję dzielników zera.
	\Odp{
		Element $a$ pewnego pierścienia, dla którego istnieje element $b$, taki że $ab=0$.
	}
	
	\item Podać formalną definicję liczb wymiernych (na bazie liczb całkowitych). Jakiej struktury jest to szczególny przypadek?
	\Odp{
	$\frac{\mathbb{Z}\times (\mathbb{Z}\setminus\emptyset)}{\sim} \qquad (m,n)\sin (a,b) \Leftrightarrow m\cdot b=n\cdot a$\\
	...
	\Odp{
	Liniowy porządek jest ciągły $\Leftrightarrow \forall_{A\neq \emptyset, \qquad A}$ ograniczony z góry, wsród ograniczeń istnieje ograniczenie najmniejsze
	}
	...
	\begin{itemize}
		...
		\item $a\in\mathbb{Q}, b\notin\mathbb{Q} \Rightarrow a\cdot b \in \mathbb{Q}$, bo gdyby $a\cdot b \in \mathbb{Q}$, to $b=\frac{1}{a} \cdot ab \in \mathbb{Q}$ sprzeczność
		\item $a,b\notin\mathbb{Q}$, to $a\cdot b$ może być i wymierne i niewymierne
	...
	\Odp{
		$$\forall_{a,b\in \mathbb{R}, a<b} \exists_{p\in \mathbb{R}\setminus\mathbb{Q}} a<p<b$$
		D: $x,y\in\mathbb{R}$
		
		$\exists_{a\in \mathbb{Q}}: x<a<y \qquad \exists_{b\in\mathbb{Q}} : a<b<y 
		\qquad\exists_{p\in \mathbb{Q}} x<a<p<b<y$
	}
